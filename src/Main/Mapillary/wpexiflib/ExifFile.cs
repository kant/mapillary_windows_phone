...
namespace ExifLibrary
{
    ...
    public class ExifFile
    {
        ...
        private void ReadAPP1()
        {
            ...
            app1 = file.Sections.FirstOrDefault(a => (a.Marker == JPEGMarker.APP1) &&
                (new AsciiEncoding().GetString(a.Header, 0, 6) == "Exif\0\0"));
            ...
            if (app1 == null)
            {
                ...
                // REPLACED WITH
                var item = file.Sections.LastOrDefault(a => a.Marker == JPEGMarker.APP0);
                int insertionIndex = item != null ? file.Sections.LastIndexOf(item) : -1;
                //END REPLACE
            ...
            IDictionary<int, IFD> ifdqueue = new Dictionary<int, IFD>();
            makerNoteOffset = 0;
            ...
            while (ifdqueue.Count != 0)
            {
                ...
                //ifdqueue.RemoveAt(0);
                var key = ifdqueue.Keys.OrderBy(it => it).First();
                int ifdoffset = tiffoffset + key;
                IFD currentifd = ifdqueue[key];
                ifdqueue.Remove(key);
                ...
                for (short i = 0; i < fieldcount; i++)
                {
                    ...
                    if (totallength > 4)
                    {
                        ...
                        mMap.Write(new Bin(ExifTagFactory.GetTagName(currentifd, tag) + " Data", 7, (int)totallength, prop));
                    }
#endif
                }
            ...
            if (mMap != null)
            {
                using (StreamReader sr = new StreamReader(mMap))
                {
        ...
        private void WriteApp1(bool preserveMakerNote)
        {
            ...
            // Exif identifer
            ms.Write(new AsciiEncoding().GetBytes("Exif\0\0"), 0, 6);
        ...
        private void WriteIFD(MemoryStream stream, Dictionary<ExifTag, ExifProperty> ifd, IFD ifdtype, long tiffoffset, bool preserveMakerNote)
        {
            ...
            while (fieldqueue.Count != 0)
            {
                ...
                ExifInterOperability interop = field.GetInteroperability(ByteOrder);
        ...
        public static ExifFile ReadStream(Stream stream)
        {
            ExifFile exif = new ExifFile();
        ...
        public void SaveStream(System.IO.IsolatedStorage.IsolatedStorageFileStream targetStream)
        {
            WriteApp1(true);

            file.Save(targetStream);

        }

        public void SaveToCameraRoll(string filename, MediaLibrary ml)
        {
            WriteApp1(true);

            file.SaveToCameraRoll(filename, ml);
        }
    }
}
